...
import { HttpErrorHandlerService } from '../../common/index';
...
export class EventService {
  ...
  public eventSaved: EventEmitter<IEvent[]>;

  public getEvents(event?: EventEmitter<IEvent[]>): Observable<IEvent[]>{
    if(event){
      this.eventSaved = event;
    }
    return this.http.get('/api/events').do((response)=> {this.eventList = response.json() as IEvent[];})
  ...
  public saveEvent(event): Observable<IEvent>{
    ...
    return this.http.post('/api/events', JSON.stringify(event), options)
      .do((response)=>{
        ...
        if(this.eventSaved){
          this.eventSaved.emit(this.eventList);  
        }
      })
